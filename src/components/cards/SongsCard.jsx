    ...
    <div className="px-0.5">
      {songsList?.map((song, index) => {
        ...
        return (
          <div
            key={id}
            className={`${isDarkMode ? 'bg-black' : ''} ${isDarkMode ? 'hover:bg-white hover:text-black' : ''} cursor-pointer hover:bg-white h-16 my-2 flex justify-between px-5 items-center border-2 rounded-lg duration-200 ${currentSongData.id === id ? "" : ""
              }  `}
